...
describe Google::Cloud::Spanner::Client, :read, :mock_spanner do
  ...
  it "deletes all rows directly with custom timeout and retry policy" do
    ...
    mock = Minitest::Mock.new
    mock.expect :create_session, session_grpc, [{database: database_path(instance_id, database_id), session: default_session_request }, default_options]
    mock.expect :commit, commit_resp, [{ session: session_grpc.name, mutations: mutations, transaction_id: nil, single_use_transaction: tx_opts, request_options: nil, precommit_token: nil }, expect_options]
    spanner.service.mocked_service = mock
  ...
  it "deletes all rows directly while excluding from change streams" do
    mutations = [
      Google::Cloud::Spanner::V1::Mutation.new(
        delete: Google::Cloud::Spanner::V1::Mutation::Delete.new(
          table: "users", key_set: Google::Cloud::Spanner::V1::KeySet.new(all: true)
    ...
    mock = Minitest::Mock.new
    mock.expect :create_session, session_grpc, [{database: database_path(instance_id, database_id), session: default_session_request }, default_options]
    mock.expect :commit, commit_resp, [{ session: session_grpc.name, mutations: mutations, transaction_id: nil, single_use_transaction: tx_opts_with_change_stream_exclusion, request_options: nil, precommit_token: nil }, default_options]
    spanner.service.mocked_service = mock
  ...
  describe "commit options" do
    ...
    it "commit using block and return commits stats" do
      ...
      mock = Minitest::Mock.new
      mock.expect :create_session, session_grpc, [{database: database_path(instance_id, database_id), session: default_session_request }, default_options]
      mock.expect :commit, commit_stats_resp_grpc, [{ session: session_grpc.name, mutations: mutations, transaction_id: nil, single_use_transaction: tx_opts, return_commit_stats: true, request_options: nil, precommit_token: nil }, default_options]
      spanner.service.mocked_service = mock
    ...
    it "commits with max_commit_delay" do
      ...
      mock = Minitest::Mock.new
      mock.expect :create_session, session_grpc, [{database: database_path(instance_id, database_id), session: default_session_request }, default_options]
      mock.expect :commit, commit_stats_resp_grpc, [{ session: session_grpc.name, mutations: mutations, transaction_id: nil, single_use_transaction: tx_opts, return_commit_stats: true, max_commit_delay: commit_delay_duration, request_options: nil, precommit_token: nil }, default_options]
      spanner.service.mocked_service = mock
  ...
  describe "priority request options" do
    let(:write_mutation){
      Google::Cloud::Spanner::V1::Mutation::Write.new(
        table: "users", columns: %w(id name active),
        values: [Google::Cloud::Spanner::Convert.object_to_grpc_value([1, "Harvey", true]).list_value]
      )
    }
    let(:request_options){
    ...
    it 'inserts records' do
      mock_commit_request [Google::Cloud::Spanner::V1::Mutation.new(insert: write_mutation )],
                          request_options: request_options do
        timestamp = client.insert "users", [{ id: 1, name: "Harvey",  active: true }],
    ...
    it 'upserts records' do
      mock_commit_request [Google::Cloud::Spanner::V1::Mutation.new(insert_or_update: write_mutation )],
                          request_options: request_options do
        timestamp = client.upsert "users", [{ id: 1, name: "Harvey",  active: true }],
                                  request_options: request_options
        _(timestamp).must_equal commit_time
      end
    end
    ...
    it 'updates records' do
      mock_commit_request [Google::Cloud::Spanner::V1::Mutation.new(update: write_mutation )],
                          request_options: request_options do
        timestamp = client.update "users", [{ id: 1, name: "Harvey",  active: true }],
                                  request_options: request_options
        _(timestamp).must_equal commit_time
      end
    end
    ...
    it 'replaces records' do
      mock_commit_request [Google::Cloud::Spanner::V1::Mutation.new(replace: write_mutation )],
                          request_options: request_options do
        timestamp = client.replace "users", [{ id: 1, name: "Harvey",  active: true }],
                                  request_options: request_options
        _(timestamp).must_equal commit_time
      end
    end
    ...
    it 'deletes records' do
      ...
      mock_commit_request mutations, request_options: request_options do
        timestamp = client.delete "users", [1, 2, 3, 4, 5], request_options: request_options
        _(timestamp).must_equal commit_time
      end
    end
    ...
    it "commits using a block" do
      ...
      mock.expect :create_session, session_grpc, [{
        database: database_path(instance_id, database_id), session: default_session_request
      }, default_options]
      mock.expect :commit, commit_resp, [{
        session: session_grpc.name, mutations: mutations, transaction_id: nil,
      single_use_transaction: tx_opts, request_options: request_options, precommit_token: nil
      }, default_options]
      spanner.service.mocked_service = mock

      timestamp = client.commit request_options: request_options do |c|
        c.update "users", [{ id: 1, name: "Charlie", active: false }]
        c.insert "users", [{ id: 2, name: "Harvey",  active: true }]
        c.upsert "users", [{ id: 3, name: "Marley",  active: false }]
        c.replace "users", [{ id: 4, name: "Henry",  active: true }]
  ...
  def mock_commit_request mutations, request_options: nil
    ...
    mock.expect :commit, commit_resp, [{
      ...
      request_options: request_options, precommit_token: nil
    }, default_options]
    spanner.service.mocked_service = mock
