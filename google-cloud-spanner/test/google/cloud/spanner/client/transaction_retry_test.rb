...
describe Google::Cloud::Spanner::Client, :transaction, :retry, :mock_spanner do
  ...
  it "raises internal error if non retryable" do
    ...
    # this test does not expect begin_transaction to be called explicitly since
    # the client does not do anything with the transaction.
    # this is the only case where `Service#commit` from `Client#transaction` is called with
    # a single-use
