...
describe Google::Cloud::Spanner::Session, :read, :mock_spanner do
  ...
  it "passes exclude_txn_from_change_streams" do
    ...
    mock = Minitest::Mock.new
    mock.expect :commit, commit_resp, [{ session: session.path, mutations: mutations, transaction_id: nil, single_use_transaction: tx_opts_with_change_stream_exclusion, request_options: nil, precommit_token: nil }, default_options]
    session.service.mocked_service = mock
  ...
  it "passes isolation_level" do
    ...
    mock = Minitest::Mock.new
    mock.expect :commit, commit_resp, [{ session: session.path, mutations: mutations, transaction_id: nil, single_use_transaction: tx_opts_isolation_level, request_options: nil, precommit_token: nil }, default_options]
    session.service.mocked_service = mock
